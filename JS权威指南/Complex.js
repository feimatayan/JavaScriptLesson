...
function Complex(real,imaginary) {
    if (isNaN(real) || isNaN(imaginary))//首先是数字
...
Complex.prototype.mul = function (that) {
    return new Complex(this.r * that.r - this.r * that.r,this.r * that.i +  this.i * that.r);
};
...
Complex.parse = function (s) {
    try {
        var  m = Complex._format.exec(s);//利用正则表达式进行批评
...
c.add(d).toString();
console.log("toString = " + c.add(d).toString());
var parseStr = Complex.parse(c.toString()).add(c.neg()).equals(Complex.ZERO);
console.log("parser = " + parseStr);